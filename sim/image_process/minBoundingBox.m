function [bbox, angle] = minBoundingBox(X)
...
% 
% Code credit:
%   Copyright (c) 2011, julien diener. All rights reserved.
%   publically available at https://www.mathworks.com/matlabcentral/fileexchange/31126-2d-minimal-bounding-box
...
Rf = Rf';
bbox(:,4) = bmax(1)*Rf(:,1) + bmin(2)*Rf(:,2);
bbox(:,1) = bmin(1)*Rf(:,1) + bmin(2)*Rf(:,2);
bbox(:,2) = bmin(1)*Rf(:,1) + bmax(2)*Rf(:,2);
bbox(:,3) = bmax(1)*Rf(:,1) + bmax(2)*Rf(:,2);
