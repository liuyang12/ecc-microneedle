import torch
import torch.nn.functional as F
from tqdm import tqdm

from utils import multiclass_dice_coeff, dice_coeff


def evaluate_binnet(net, dataloader, device):
    net.eval()
    num_val_batches = len(dataloader)
    dice_score = 0

    # iterate over the validation set
    with tqdm(total=num_val_batches, desc='Evaluation round', unit='batch', leave=False) as pbar:
        for idx, (image, mask) in enumerate(dataloader):
            # move images and labels to correct device and type
            image = image.to(device=device, dtype=torch.float32)
            mask = mask.to(device=device, dtype=torch.long)
            mask = F.one_hot(mask, net.out_channels).permute(0, 3, 1, 2).float()

            with torch.no_grad():
                # predict the mask
                mask_pred = net(image)

                # convert to one-hot format
                if net.out_channels == 1:
                    mask_pred = (F.sigmoid(mask_pred) > 0.5).float()
                    # compute the Dice score
                    dice_score += dice_coeff(mask_pred, mask, reduce_batch_first=False)
                else:
                    mask_pred = F.one_hot(mask_pred.argmax(dim=1), net.out_channels).permute(0, 3, 1, 2).float()
                    # # compute the Dice score, ignoring background
                    # dice_score += multiclass_dice_coeff(mask_pred[:, 1:, ...], mask[:, 1:, ...], reduce_batch_first=False)
                    # should not ignore background for binary classifications
                    dice_score += multiclass_dice_coeff(mask_pred[:, :, ...], mask[:, :, ...], reduce_batch_first=False)

           

    net.train()
    return dice_score / num_val_batches
